...
- **MQTT 远程控制**：适用于纯数据物联卡（无短信/通话权限），通过公共 MQTT Broker 收发指令。多节点故障转移，Client ID 使用 IMEI 标识设备，支持 3 条指令（reboot/reconnect/status）与可选 Token 鉴权。节点的主机/端口/TLS 均可逐条编辑，页面内置独立日志视图。冷启动通过蜂窝网卡 IP 就绪门禁确保网络真正可用后才建连，彻底替代 TCP 探测方案。
...
- **国内公共 Broker**：预设 3 个公共服务节点（EMQX / Mosquitto），无需自建服务器
...
- **指数退避上限 10 秒**：会话中断退避与节点不可达退避均以 10 秒为上限，避免长期运行后重连间隔过长
...
- **会话中断与节点不可达分开处理**：连上之后掉线（broker 踢 client_id、网络抖动、发布超限）走独立的重连路径——原地重连**同一个节点**，不轮换到别的节点，也不累加「全部节点不可达」的指数退避；退避上限 10 秒，会话稳定运行超过 60 秒后计数归零。旧实现把两者混为一谈，日志会误报「节点连接失败 / 全部节点不可达」，而那个节点其实是通的
