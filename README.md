...
从已构建的 `3.4.2` 设备升级本次内存显示精度修复时，请使用 GitHub 生成的 `3.4.3` 包：在设备管理页面选择 **OTA 更新**，上传 `.tar.gz`，确认验证结果为“通过”，然后选择“应用并重启”。请继续保留当前可用的 `3.4.2` GitHub OTA 包，以便需要时手动上传恢复。
...
Dashboard 主百分比也显示为同一口径的“可用内存百分比”。详情会区分进程及不可回收内核占用、可回收缓存和共享内存，避免把 Linux 文件缓存误解为不可回收的应用占用。
