...
| 接口 | 方法 | 说明 |
|---|---|---|
| `/api/health` | GET | 健康检查（无需认证）。仅 ofono/数据库参与判定；MQTT 状态作明细返回（`affects_health=false`），不影响整体健康 |
...
- 实时网速、信号强度、连接状态
- 可用内存百分比（Linux `MemAvailable` 语义，上限 100%）
- CPU 温度、磁盘使用、运行时间
- 警告/错误日志计数快捷入口
- 自适应轮询（页面可见时高频，隐藏时降频）
...
- **默认出厂化（无个人数据）**：代码与 OTA 包不内置任何私有凭据。配置文件 `config.json` 与短信/通话库 `data.db` 只存在于目标设备自己的 `/data` 分区，属于设备私有运行态数据，**永远不打包进源码仓库，也不打包进 OTA 升级包**。首次启动无 `config.json` 时，全部参数回落到「空凭据 + 公共 Broker 节点 + 默认主题」的通用出厂值
- **解析失败兜底（防配置清空）**：升级只覆盖程序文件，不触碰用户已有的 `config.json`。即便目标设备的旧配置因缺字段或断电截断而解析失败，系统也会先把原文件备份为 `config.json.corrupt` 再回落到出厂默认值，**绝不静默清空用户配置**；`deny_unknown_fields` 被显式禁止，未来新增字段前向兼容（旧设备加载新配置不报错）
...
- **公共 Broker**：内置若干公共节点（EMQX / Mosquitto / Eclipse / HiveMQ，含明文与 TLS 两种端口），无需自建服务器；节点列表可逐条增删改
...
- **单包上限显式放开到 64KB**：rumqttc 默认单包上限仅 10KB，而完整状态报告 JSON 约 10.3KB，刚好越线。旧实现在发布状态时会被本地拦下，报 `Cannot send packet of size '10315'. It's greater than the broker's maximum packet size of: '10240'` 并断连重连——**注意这条错误文案里的 "broker's" 是 rumqttc 的措辞误导，比对的其实是客户端自己的 `max_outgoing_packet_size`，与 broker 无关**。表现为「连上就掉、反复重连、日志刷满」
- **会话中断与节点不可达分开处理**：连上之后掉线（broker 踢 client_id、网络抖动、发布超限）走独立的重连路径——原地重连**同一个节点**，不轮换到别的节点，也不累加「全部节点不可达」的指数退避；退避上限 30 秒，会话稳定运行超过 60 秒后计数归零。旧实现把两者混为一谈，日志会误报「节点连接失败 / 全部节点不可达」，而那个节点其实是通的
- **配置热生效**：连接期间修改节点/端口/主题/凭据，5 秒内自动断开并按新配置重连（仅比较真正影响连接的字段，避免无谓重连抖动）
- **不拖垮系统健康判定**：`/api/health` 只判定进程自身的核心依赖（ofono D-Bus、SQLite），MQTT 连接状态作为明细返回并标记 `affects_health=false`。broker 是外部第三方服务，它不可达并不代表本后端进程有问题；旧实现会让「系统配置」页面显示 **系统异常 / 后端服务异常**，与事实相反并把排障方向指向一个健康的进程。该页面现在按 **系统正常 / 部分依赖异常 / 后端无响应** 三态显示，并逐个列出组件状态
...
OTA 的打包、上传与安装三层都保证「刷机只刷程序，不碰用户配置」：

- **打包层**：`pack-ota.sh` 只把 `meta.json + udx710 + www/` 打进 tar.gz，顶层多任何一项即硬失败；`config.json`、`data.db`、`net_health_state.json`、`mode.cfg` 都不在包里。
- **上传层**：后端解压校验只放行 `meta.json`、`udx710`、`www/` 三个顶层入口，其余一律拒绝，从协议上封死「包里夹带配置文件覆盖用户数据」的可能。
- **安装层**：`install_update` 的 rename 切换只作用于 `/home/root/udx710` 与 `/home/root/www`；唯一额外动作是在设备尚无启动脚本时补一个空 `init.sh` 占位，与 `/data/config.json`、`/data/data.db` 无关，二者永不读取、永不改写。
- **兼容性兜底**：设备实现在线升级后，旧 `config.json` 直接沿用。若旧配置缺失本版本新增字段，`#[serde(default)]` + `sanitize()` 在加载期补齐默认值；解析失败则备份后回落默认值，前端因所有配置请求都有空值兜底 + 独立 try/catch，不会白屏。上面的「配置安全 → 解析失败兜底」小节有参数级说明。
...
你可以自由使用、研究、修改和分发本项目；但分发源代码或衍生作品（含 OTA 产物）时，必须：

1. 保留版权声明、许可证文本和修改说明（含源码文件头的 `@Author: 1orz` 署名）
2. 以 GPLv3 公开相应完整源代码
3. 以 GPLv3 发布衍生作品
