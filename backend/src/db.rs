...
impl Database {
    ...
    pub fn insert_sms(
        ...
        let conn = self.lock_conn();
        let timestamp = crate::utils::beijing_now_rfc3339();
    ...
    pub fn insert_call(
        ...
        let conn = self.lock_conn();
        let start_time = crate::utils::beijing_now_rfc3339();
    ...
    pub fn add_traffic_delta(&self, rx_delta: u64, tx_delta: u64) -> Result<()> {
        let conn = self.lock_conn();
        let today = crate::utils::beijing_now().format("%Y-%m-%d").to_string();
    ...
    pub fn get_months_traffic(&self) -> Result<(u64, u64)> {
        let conn = self.lock_conn();
        let month_prefix = crate::utils::beijing_now().format("%Y-%m").to_string();
