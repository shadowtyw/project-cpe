...
pub fn switch_usb_mode_advanced(mode: u8) -> Result<(), String> {
    ...
    // 成功路径也做一次验证：确认 UDC 已绑定且 usb0 有 IP 地址。
    // 若验证失败则触发恢复。
    if let Err(verify_error) = verify_usb_gadget_healthy(&usb_interface_ip) {
        recover_usb_after_failed_switch(&udc_name_cached, &usb_interface_ip, &verify_error);
...
/// 验证 USB gadget 切换后状态正常：UDC 已绑定且 usb0 有 IP 地址。
fn verify_usb_gadget_healthy(usb_interface_ip: &str) -> Result<(), String> {
    // 检查 UDC 是否已绑定（非 "none"）
    let udc_content = fs::read_to_string(UDC_PATH)
        .map_err(|e| format!("Failed to read UDC status: {}", e))?;
    let udc = udc_content.trim();
    if udc.is_empty() || udc == "none" {
        return Err("UDC not bound after USB mode switch".to_string());
    }

    // 检查 usb0 接口是否已有配置的 IP 地址
        ...
        .output()
    {
        if output.status.success() {
            ...
            if !text.contains(usb_interface_ip) {
                return Err(format!(
                    "usb0 IP {} not found after USB mode switch",
                    usb_interface_ip
                ));
            }
        }
