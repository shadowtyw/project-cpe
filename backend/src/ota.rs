...
fn install_update(staging_binary: &Path, staging_www: &Path) -> Result<(), String> {
    ...
    // 写入安装哨兵：进程崩溃/断电后，下次启动自动回滚到备份。
    let _ = fs::write(OTA_INSTALLING_SENTINEL, "");
    ...
    if let Err(error) = install_result {
        ...
        let _ = fs::remove_file(OTA_INSTALLING_SENTINEL);
        return Err(error);
    }
...
/// 启动时调用：检查是否有中断的 OTA 安装，若有则自动回滚到备份。
///
/// 当进程在 `install_update` 的 rename 阶段崩溃（或设备断电）时，哨兵文件
/// `/tmp/ota_installing` 会残留。下次启动时调用此函数，自动从备份恢复。
pub fn recover_interrupted_ota() {
    if !Path::new(OTA_INSTALLING_SENTINEL).exists() {
        return;
    }
    ...
    // 从备份恢复
    let backup_binary = Path::new(OTA_BACKUP_DIR).join("udx710");
    let backup_www = Path::new(OTA_BACKUP_DIR).join("www");
    if backup_binary.is_file() && backup_www.is_dir() {
        if let Err(e) = install_update(&backup_binary, &backup_www) {
            tracing::error!(error = %e, "Failed to recover interrupted OTA from backup");
        } else {
            tracing::info!("Recovered interrupted OTA from backup snapshot");
        }
    } else {
        // 没有可用备份：尝试从 .old 路径恢复（rename 阶段中断的中间状态）
        if Path::new(OTA_OLD_BINARY_PATH).exists() {
            let _ = fs::rename(OTA_OLD_BINARY_PATH, OTA_BINARY_PATH);
        }
        if Path::new(OTA_OLD_WWW_PATH).exists() {
            let _ = fs::rename(OTA_OLD_WWW_PATH, OTA_WWW_PATH);
        }
        let _ = crate::config::ensure_loader_hooks_init();
        tracing::warn!("Interrupted OTA recovered from .old fallback (no backup snapshot available)");
    }
}
